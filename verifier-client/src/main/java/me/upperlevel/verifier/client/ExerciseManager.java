...
public class ExerciseManager {
    ...
    protected Map<String, ExerciseHandler<?>> exercises = new HashMap<>();

    protected Map<String, Consumer<ExerciseHandler<?>>> listeners = new HashMap<>();
    ...
    public void loadSaved() {
        if(EX_FOLDER == null) {
            Main.getUI().error(new IllegalStateException("The exercises folder isn't initialized"));
            return;
        }
        File[] files = EX_FOLDER.listFiles();
        if(files == null) {
            Main.getUI().error(new IllegalStateException("The EX_FOLDER is a directory!"));
            return;
        }
        for (File file : files) {
            try {
